...
description: 当用户给出模糊、含混、藏假设的需求,或疑似 X-Y problem(问的是 Y、真正的问题是 X),需要先澄清再动手时使用。典型信号：用户说"给我做个 X""帮我弄一下""我想要个功能,要支持..."但关键细节缺失、用户甩出一句话需求就期望开做、用户描述很详细但藏了隐含假设、用户要的方案可能不是真正需要的方案(X-Y problem)、用户脑里似乎有想法但表达不全、需求模糊到无法判断该做什么。涉及关键词：需求澄清、问需求、澄清问题、需求不明确、需求模糊、问清楚、明确需求、需求分析、X-Y problem、xy problem、隐含假设、需求挖掘、需求确认、反问、追问。
---
