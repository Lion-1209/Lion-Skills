...
**通过标准**：subagent 的产出 (a) 解决了用户问题，且 (b) 遵循 skill 的设计意图（该走流程的走了、该遵循的约定遵循了）。卡壳或跑偏 → 改正文重跑。

**把用例固化下来**：把这几个 prompt + 预期结果写进 `skills/<name>/evals/evals.json`（格式：`{skill_name, evals: [{id, prompt, expected}]}`）。evals.json 是验证的"存档"（记录测过什么、预期什么），subagent 实跑是"当下执行"——两者配合：先定 evals.json，再用 subagent 跑这些 prompt，核对产出是否达到 expected。
